...
6. **Memory** — используется ТОЛЬКО `~/.claude/projects/.../memory/MEMORY.md` (автоматический файл Claude Code, загружается в системный промпт каждой сессии). **Автосохранение**: после КАЖДОГО завершённого шага (фикс, фича, деплой, аудит, изменение конфига) НЕМЕДЛЕННО записывать результат в MEMORY.md. Не ждать конца сессии, не батчить — сохранять СРАЗУ после каждого действия. Файл `memory.md` в корне проекта НЕ используется.
